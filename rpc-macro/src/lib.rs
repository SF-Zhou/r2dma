...
pub fn service(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    for item in &input_items {
        if let TraitItem::Fn(method) = item {
            ...
            if *method_ident == "export_interface" {
                panic!("Functions cannot be named `export_interface`!");
            }
            ...
            client_methods.push(quote! {
                async fn #method_ident(&self, ctx: &#krate::Context, req: #req_type) -> #rsp_type {
                    self.call(ctx, req, #method_name).await
                }
            });
            ...
            invoke_branchs.push(quote! {
                ...
                map.insert(
                    ...
                    Box::new(move |ctx, meta, bytes| {
                        let req = Deserialize::deserialize(bytes)?;
                        let this = this.clone();
                        let ctx = ctx.clone();
    ...
    quote! {
        #visibility trait #trait_ident {
            ...
            fn export_interface(
                ...
                String,
                Box<dyn Fn(&#krate::Context, #krate::Meta, &[u8]) -> #krate::Result<()>>
            >
            where
...
pub(crate) fn get_crate_name() -> proc_macro2::TokenStream {
    let found_crate = proc_macro_crate::crate_name("rpc").unwrap_or_else(|err| {
